...
### 11. AddressLimitExceeded: Maximum number of addresses reached
...
**Check quota:**
```bash
# Count current EIPs (Free Tier limit: 5)
aws ec2 describe-addresses --region ap-southeast-1 --query 'length(Addresses)' --output text
```

---

### 12. Terraform exited with code 1 (Generic error)

**This is NOT the actual error!** Scroll up in logs to find real error.
...
# If complete failure, check specific error above
```

---

### 13. Terraform plan/apply timeout in GitHub Actions

**Quick Fix:**
- Timeouts đã được thêm vào workflow
- Re-run workflow
- Hoặc chạy local:
